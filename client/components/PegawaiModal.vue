      ...
      <form @submit.prevent="handleSubmit" class="p-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <p v-if="!masterData?.tempat?.length" class="text-xs text-red-500 mt-1">
              Data tempat belum dimuat
            </p>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option 
                v-for="g in (masterData?.golongan || [])" 
                :key="g.id" 
                :value="g.id"
              >
                {{ g.kode_golongan }}
          ...
          <div>
            ...
            <p v-if="!masterData?.jabatan?.length" class="text-xs text-red-500 mt-1">
          ...
          <div>
            ...
            <p v-if="!unitKerjaList?.length" class="text-xs text-red-500 mt-1">
              Data unit kerja belum dimuat
            </p>
          </div>
...
onMounted(() => {
  console.log('PegawaiModal - masterData:', props.masterData)
  console.log('PegawaiModal - unitKerjaList:', props.unitKerjaList)
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.mode === 'add') {
      const response = await api.createPegawai(formDataToSend)
      console.log('✅ Server response:', response)
      alert('Data berhasil ditambahkan')
    } else if (props.mode === 'edit') {
      formDataToSend.append('_method', 'PUT')
      const response = await api.updatePegawai(props.pegawai.id, formDataToSend)
      console.log('✅ Server response:', response)
    ...
    const errorMsg = error.data?.message || error.message || 'Gagal menyimpan data'
